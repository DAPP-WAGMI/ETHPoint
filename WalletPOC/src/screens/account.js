...
class Account extends Component {
    ...
    async componentDidMount() {
        const maticBalance = this.web3.utils.fromWei(await this.web3.eth.getBalance(this.web3.eth.accounts.privateKeyToAccount(this.context.value.priv).address), 'ether')
        const USDCBalance = await this.getBalanceToken(this.web3.eth.accounts.privateKeyToAccount(this.context.value.priv).address, USDCaddress)
        const USDTBalance = await this.getBalanceToken(this.web3.eth.accounts.privateKeyToAccount(this.context.value.priv).address, USDTaddress)
        this.setState({
            maticBalance,
            USDCBalance,
            USDTBalance
        ...
        fetch(`https://api.polygonscan.com/api?module=account&action=txlist&address=${this.web3.eth.accounts.privateKeyToAccount(this.context.value.priv).address}&startblock=0&endblock=99999999&sort=desc&apikey=EY9AH8AUA89IRBBWKDEIXE1CVQFP3I5E64`, requestOptions)
                    ...
                    <View style={[GlobalStyles.main, { flexDirection: "column", alignItems: "center", paddingTop: 20 }]}>
                        ...
                        <View style={{ borderBottomWidth: 0.5, borderColor: "black", width: "80%", paddingBottom: 20 }}>
                            <Text style={{ textAlign: "center", color: "black", fontSize: 20, }}>
                                Address: {"\n"}
                                {
                                    this.web3.eth.accounts.privateKeyToAccount(this.context.value.priv).address
                                }
                            </Text>
                        </View>
